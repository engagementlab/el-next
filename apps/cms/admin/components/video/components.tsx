...
export const Field = ({
  ...
  const [useStore] = useState(() => 
      create<VideoGridState>(set => ({
        ...
        setVideo: (video: RelatedVideo) => set((state) => {
          console.log(video.label, _.map(state.currentVideos, 'value').includes(video.value))
  ...
  const currentVideos: RelatedVideo[] = useStore((state: { currentVideos: any; }) => state.currentVideos);
    ...
    <FieldContainer>
      <FieldLabel>{field.label}</FieldLabel>
        <Fragment>
          ...
          <VideoSelector videos={currentVideos} data={data} open={gridOpen} 

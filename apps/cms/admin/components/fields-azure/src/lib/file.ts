...
const views = './admin/components/fields-azure/src/views/file';
...
const AzureStorageFileFieldOutputType = graphql.object<
  Omit<FileData, 'type'>
>()({
...
function createInputResolver(config: AzureStorageConfig) {
  return async function inputResolver(
    data: AzureStorageFieldInputType,
    ...
    if (data.ref) {
      if (data.upload) {
        throw new Error(
          'Only one of ref and upload can be passed to AzureStorageFileFieldInput'
        );
      }
    ...
    return fieldType({
      ...
      input: {
        create: {
          ...
          resolve: createInputResolver(
            azureStorageConfig as AzureStorageConfig
          ),
        },
        update: {
          arg: graphql.arg({ type: AzureStorageFileFieldInput }),
      ...
      unreferencedConcreteInterfaceImplementations: [
        AzureStorageFileFieldOutputType,
      ],
      views,
    });
  };

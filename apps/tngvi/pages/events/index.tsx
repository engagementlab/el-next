...
const Item = (props: ItemProps) => {
  return (
    <div
      className="w-full flex flex-col-reverse md:flex-row mb-20"
      key={`${props.past ? '' : 'upcoming'}-${props.index}`}
    >
      <div className="w-full md:w-1/3">
        ...
        {!props.past && (
          <div>
            {props.event.address && (
              <div className="my-4 mt-8 font-medium">{props.event.address}</div>
            )}
            {props.event.registrationLink && (
              <a href={props.event.registrationLink}>
                <button className="inline-block rounded-full px-8 py-6 uppercase bg-lynx text-bluegreen border-2 border-bluegreen transition-all hover:bg-bluegreen hover:text-lynx">
                  Register Now
                </button>
              </a>
            )}
          ...
          <div className="cursor-pointer">
            <a className="group">
              ...
              {props.event.thumbnail ? (
                <Image
                  id={`thumb-${props.index}-${props.past ? '' : 'upcoming'}`}
                  alt={`Thumbnail for event with name "${props.event.name}" `}
                  imgId={props.event.thumbnail.publicId}
                  width={335}
                />
              ) : (
                <ImagePlaceholder
  ...
  return (
    <Layout>
      <div className="container mt-14 mb-24 xl:mt-16 px-4 xl:px-8">
        ...
        <h2 className="text-2xl text-bluegreen font-semibold mt-20">
          Past Events
        </h2>
        <div className="flex flex-col mt-6">
          {past.map((event, i) => (

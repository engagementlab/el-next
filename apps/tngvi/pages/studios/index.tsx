...
const renderItem = (props: {
  ...
  const btnClass =
    'my-4 lg:my-0 inline-block rounded-full px-8 py-5 uppercase bg-lynx text-bluegreen border-2 border-bluegreen transition-all hover:bg-green-blue hover:text-lynx group-hover:bg-green-blue group-hover:text-lynx hover:border-green-blue group-hover:border-green-blue';
  return (
      ...
      >
        {props.item.thumbnail ? (
          <Image
            ...
            alt={`Thumbnail for studio with name "${props.item.name}"
                        `}
        ...
        <div className="flex flex-col lg:flex-row items-start justify-between">
          <div className="w-full lg:w-2/3">
            <p
              ...
              dangerouslySetInnerHTML={{
                __html: props.item.blurb.replace(
                  'Facilitated by',
                  '<i>Facilitated by</i>'
                ),
              }}
          ...
          <button className={btnClass}>See Details</button>
        </div>
      </motion.div>
    </Link>
  );
...
export default function Studios({
  ...
  const preSelectedFilters =
    Object.keys(router.query).length === 1
      ...
      : [];
  const filtering = new Filtering<StudioItem>(
    filtersGrouped,
    preSelectedFilters,
    studios,
    renderItem
    ...
    <Layout error={error}>
      <div className="container mt-14 mb-24 xl:mt-16 px-4 xl:px-8">
        ...
        <p className="w-full lg:w-1/2 mb-14">
          Students and faculty work alongside community partners to co-create
          narrative interventions to the crisis of gun violence as it is
          experienced locally. The{' '}
          <i>Transforming Narratives of Gun Violence Initiative</i> is a
          multi-year initiative and hosts 5-7 studios per year.
        </p>

...
export default function Studios({
  ...
  let filteredItems = studios || null;
          ...
          filteredItems = studios
            ? studios.filter((item) => {
                ...
                return (
                  ...
                  item.initiatives[0].toLowerCase() === group
                );
              })
            : [];
          return {
  ...
  if (router.asPath.indexOf('?') > 0) {
    filter = router.asPath
      .substring(router.asPath.indexOf('?'), router.asPath.length)
      .replace('?', '');
  }
  ...
  // Store get/set
  const { currentTheme, selectedFilter } = useStore((state) => state);
  const noGroupsOpen = () => {

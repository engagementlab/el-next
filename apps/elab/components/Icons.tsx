import * as React from 'react';

type Props = {
  icons: string[] | undefined;
  className?: string | undefined;
};
export const Icons = ({ icons, className }: Props): JSX.Element => {
  return (
    <span className={className || ''}>
      {icons?.includes('news') && (
        <svg viewBox="120 -840 48 50.014" width="48" height="30">
          <path d="M 124 -789.986 C 122.933 -789.986 122 -790.402 121.2 -791.236 C 120.4 -792.069 120 -793.042 120 -794.153 L 120 -835.832 C 120 -836.944 120.4 -837.916 121.2 -838.75 C 122 -839.583 122.933 -840 124 -840 L 154.8 -840 L 168 -826.246 L 168 -794.153 C 168 -793.042 167.6 -792.069 166.8 -791.236 C 166 -790.402 165.067 -789.986 164 -789.986 L 124 -789.986 Z M 124 -794.153 L 164 -794.153 L 164 -823.924 L 152.6 -823.924 L 152.6 -835.832 L 124 -835.832 L 124 -794.153 Z M 130.6 -801.864 L 157.4 -801.864 L 157.4 -806.032 L 130.6 -806.032 L 130.6 -801.864 Z M 130.6 -823.954 L 144 -823.954 L 144 -828.122 L 130.6 -828.122 L 130.6 -823.954 Z M 130.6 -812.909 L 157.4 -812.909 L 157.4 -817.077 L 130.6 -817.077 L 130.6 -812.909 Z M 124 -835.832 L 124 -823.924 L 124 -835.832 L 124 -794.153 L 124 -835.832 Z"></path>
        </svg>
      )}
      {icons?.includes('event') && (
        <svg height="48" viewBox="0 -960 960 960" width="48">
          <path d="M596.817-220Q556-220 528-248.183q-28-28.183-28-69T528.183-386q28.183-28 69-28T666-385.817q28 28.183 28 69T665.817-248q-28.183 28-69 28ZM180-80q-24 0-42-18t-18-42v-620q0-24 18-42t42-18h65v-60h65v60h340v-60h65v60h65q24 0 42 18t18 42v620q0 24-18 42t-42 18H180Zm0-60h600v-430H180v430Zm0-490h600v-130H180v130Zm0 0v-130 130Z" />
        </svg>
      )}
      {(icons?.includes('documentaryfilm') ||
        icons?.includes('documentary')) && (
        <svg height="48" viewBox="0 -960 960 960" width="48">
          <path d="M160-120v-720h60v60h120v-60h280v60h120v-60h60v720h-60v-60H620v60H340v-60H220v60h-60Zm60-120h120v-120H220v120Zm0-180h120v-120H220v120Zm0-180h120v-120H220v120Zm400 360h120v-120H620v120Zm0-180h120v-120H620v120Zm0-180h120v-120H620v120ZM400-180h160v-600H400v600Zm0-600h160-160Z" />
        </svg>
      )}
      {(icons?.includes('play') || icons?.includes('video')) && (
        <svg viewBox="78.433 -801.039 800 640" width="48" height="48">
          <path
            d="M 391.277 -313 L 651.277 -482 L 391.277 -651 L 391.277 -313 Z"
            transform="matrix(1, 0, 0, 1, -0.722914, 0.710661)"
            className="transition-all group-hover:scale-110 origin-[70%_-60%]"
          ></path>
          <path d="M 138.433 -161.039 C 122.433 -161.039 108.433 -167.039 96.433 -179.039 C 84.433 -191.039 78.433 -205.039 78.433 -221.039 L 78.433 -741.039 C 78.433 -757.039 84.433 -771.039 96.433 -783.039 C 108.433 -795.039 122.433 -801.039 138.433 -801.039 L 818.433 -801.039 C 834.433 -801.039 848.433 -795.039 860.433 -783.039 C 872.433 -771.039 878.433 -757.039 878.433 -741.039 L 878.433 -221.039 C 878.433 -205.039 872.433 -191.039 860.433 -179.039 C 848.433 -167.039 834.433 -161.039 818.433 -161.039 L 138.433 -161.039 Z M 138.433 -221.039 L 818.433 -221.039 L 818.433 -741.039 L 138.433 -741.039 L 138.433 -221.039 Z"></path>
        </svg>
      )}
      {icons?.includes('article') && (
        <svg height="48" width="48" viewBox="0 -960 960 960">
          <path d="M180-120q-24 0-42-18t-18-42v-600q0-24 18-42t42-18h462l198 198v462q0 24-18 42t-42 18H180Zm0-60h600v-428.571H609V-780H180v600Zm99-111h402v-60H279v60Zm0-318h201v-60H279v60Zm0 159h402v-60H279v60Zm-99-330v171.429V-780v600-600Z" />
        </svg>
      )}
      {icons?.includes('action') && (
        <svg height="48" width="48" viewBox="0 -960 960 960">
          <path d="m438-441 192-192-43-43-149 149-65-65-43 43 108 108Zm42 282q133-121 196.5-219.5T740-552q0-118-75.5-193T480-820q-109 0-184.5 75T220-552q0 75 65 173.5T480-159Zm0 79Q319-217 239.5-334.5T160-552q0-150 96.5-239T480-880q127 0 223.5 89T800-552q0 100-79.5 217.5T480-80Zm0-480Z" />
        </svg>
      )}
      {icons?.includes('website') && (
        <svg height="48" viewBox="0 -960 960 960" width="48">
          <path d="M 180 -120 C 164 -120 150 -126 138 -138 C 126 -150 120 -164 120 -180 L 120 -780 C 120 -796 126 -810 138 -822 C 150 -834 164 -840 180 -840 L 459 -840 L 459 -780 L 180 -780 L 180 -180 L 780 -180 L 780 -459 L 840 -459 L 840 -180 C 840 -164 834 -150 822 -138 C 810 -126 796 -120 780 -120 L 180 -120 Z"></path>
          <path
            d="M 382 -339 L 340 -382 L 738 -780 L 519 -780 L 519 -840 L 840 -840 L 840 -519 L 780 -519 L 780 -737 L 382 -339 Z"
            className="transition-all group-hover:translate-x-5 group-hover:-translate-y-5"
          ></path>
        </svg>
      )}
      {icons?.includes('app') && (
        <svg height="48" width="48" viewBox="0 -960 960 960">
          <path
            d="M 480 -313 L 287 -506 L 330 -549 L 450 -429 L 450 -800 L 510 -800 L 510 -429 L 630 -549 L 673 -506 L 480 -313 Z"
            className="transition-all group-hover:translate-y-10"
          ></path>
          <path d="M 220 -160 C 204 -160 190 -166 178 -178 C 166 -190 160 -204 160 -220 L 160 -363 L 220 -363 L 220 -220 L 740 -220 L 740 -363 L 800 -363 L 800 -220 C 800 -204 794 -190 782 -178 C 770 -166 756 -160 740 -160 L 220 -160 Z"></path>
        </svg>
      )}
      {icons?.includes('book') && (
        <svg viewBox="0 -960 960 960" height="20" width="20">
          <path d="M290-80q-53.857 0-91.929-38.071Q160-156.143 160-210v-540q0-53.857 38.071-91.929Q236.143-880 290-880h510v600q-26 0-43 21t-17 49q0 28 17 49t43 21v60H290Zm-70-240q15-10 32.5-15t37.5-5h30v-480h-30q-29.167 0-49.583 20.417Q220-779.167 220-750v430Zm160-20h360v-480H380v480Zm-160 20v-500 500Zm69.541 180H699q-9-15-14-33t-5-37q0-20 5-37.5t15-32.5H289.607Q261-280 240.5-259.583 220-239.167 220-210q0 29 20.5 49.5t49.041 20.5Z" />
        </svg>
      )}
      {icons?.includes('arrow') && <>➝</>}
    </span>
  );
};

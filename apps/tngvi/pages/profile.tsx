...
export default function GetInvolved() {
  ...
  const SubmitProfile = async (
    ...
    const url = edit
      ? `${functionUrl}/orchestrators/UserEditOrchestrator`
      ...
      const makeRequest = async (img: string | ArrayBuffer | null = null) => {
        try {
          ...
          if (edit)
            params.append(
              'token',
              (form.querySelector('#token') as HTMLInputElement).value
            );
          ...
          params.append(
            'blurb',
            (form.querySelector('#blurb') as HTMLInputElement).value
          );
          params.append(
            'remembrance',
            (form.querySelector('#remembrance') as HTMLInputElement).value
          );

          if (img) params.append('img', img as string);
  ...
  const SubmitUserToken = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const requestResult = await axios.get(
        `${functionUrl}/PersonProfileGet?token=${token}`
      );
          ...
          <div className="w-full">
                ...
                <div>
                  <div className="flex flex-col">
                    <input
                      ...
                      id="token"
                      width="200"
                      aria-label="Enter your full token"
                      minLength={6}
                      maxLength={6}
                      required
                      disabled={editStatus === 'submitted'}
                      className={`bg-lynx placeholder:text-bluegreen
                                        py-4 px-4 border-2 max-w-[200px] rounded-full transition-all ${
                                          editStatus == 'no_user'
                                            ? 'border-[#F4B477]'
                                            : 'border-bluegreen'
                                        }`}
                    ...
                    {editStatus !== 'submitted' ? (
                      <div>
                        <input
                          type="submit"
                          value="Submit your token"
                          name="submit"
                          aria-hidden="true"
                          className="hidden"
                        />
                        <button
                          className={`inline-block rounded-full px-10 py-2 mt-4 uppercase border-2
            ...
            {editStatus === 'form' && (
              <form
                id="edit"
                ref={editFormRef}
                onSubmit={(e) => SubmitProfile(e, 'edit')}
              >
                <div>
                  <h3 className="text-2xl text-bluegreen font-semibold mb-8">
                    Edit your profile
                  </h3>
                  <div className="flex flex-col">
                    <p>*Required</p>
                    ...
                    <p className="text-purple font-semibold">
                      First and last name you'd like displayed, including
                      preferred prefix or suffixes*
                    </p>
                    <input
                      type="text"
                    ...
                    />
                    <p className="text-purple font-semibold">
                      Title and Organization/affiliation you'd like listed (if
                      applicable)*
                    </p>
                    ...
                    />
                    <p className="text-purple font-semibold">
                      In remembrance of: (Name, birth and death date of someone
                      close to you killed by gun violence) if applicable
                    </p>
                    <input
                      type="text"
                      placeholder="YOUR REMEMBRANCE"
                      name="remembrance"
                      id="remembrance"
                      width="800"
                      maxLength={100}
                      aria-label="Enter your remembrance (optional)"
                      minLength={5}
                      disabled={submitted}
                      defaultValue={formData['remembrance']}
                      className="w-full bg-lynx placeholder:text-bluegreen py-4 px-4 border-2 rounded-full
                                                transition-all border-bluegreen mt-4"
                    />
                    <p className="text-purple font-semibold">
                      Why are you involved in TNGV? (800 characters max)*
                    </p>
                    <textarea
                      placeholder="WHY ARE YOU INVOLVED*"
                      name="blurb"
                      id="blurb"
                    ...
                    />
                    <img
                      src={`https://res.cloudinary.com/engagement-lab-home/image/upload/c_scale,f_auto,w_250/${formData['image']._meta.public_id}`}
                    ...
                    <div
                      {...getRootProps({
                        className:
                          'dropzone bg-sorbet/30 p-4 mt-4 rounded-large cursor-pointer',
                      })}
                    >
                      <input {...getInputProps()} />
                      <p>
                        <b>To change your bio image</b>: Drag and drop a bio
                        image here, or click to select one.*{' '}
                    ...
                    {!submitted ? (
                      <button
                        type="submit"
                        className={`inline-block rounded-full px-10 py-2 mt-4 uppercase border-2
                                                border-oasis text-purple text-sm lg:text-lg transition-all max-w-[200px] hover:bg-oasis`}
            ...
            <form onSubmit={(e) => SubmitProfile(e, 'new')}>
              <div>
                {(status === '' || status !== 'success') &&
                  editStatus !== 'form' && (
                    <div>
                      <h3 className="text-1xl text-bluegreen">
                      ...
                      <div className="flex flex-col">
                        ...
                        />
                        <p className="text-purple font-semibold">
                          Title and Organization/affiliation you'd like listed
                          (if applicable)*
                        </p>
                        <input
                          type="text"
                        ...
                        />
                        <p className="text-purple font-semibold">
                          In remembrance of: (Name, birth and death date of
                          someone close to you killed by gun violence) if
                          applicable
                        </p>
                        <input
                          type="text"
                        ...
                        >
                          ...
                          <p>
                            <strong>
                              Drag and drop a bio image here, or click to select
                              one.*
                            </strong>{' '}
                            Please try to use a high-quality image.{' '}
                            <em>(Only *.jpeg and *.png images accepted)</em>
                          </p>
                  ...
                  <div>
                    {status === 'success' && (
                      <div className="text-purple">
                        ...
                        <p>
                          <p>
                            You will need the following token to login to edit
                            your profile in the future. Please treat this token
                            as you would a password.
                          </p>
                          <p>Your token is:</p>
                          <p className="text-green-blue text-5xl font-extrabold">
                            {userToken}
                          </p>
                        </p>
                      </div>

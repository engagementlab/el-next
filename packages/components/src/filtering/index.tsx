...
export default class Filtering<T> {
  ...
  useStore: UseBoundStore<
    ...
    Omit<StoreApi<FilterState>, 'subscribe'> &
      StoreSubscribeWithSelector<FilterState>
  >;
  filtersGrouped: {
    [x: string]: any[];
  };
  ...
  private RenderFilters(filters: { [x: string]: any[] }) {
    ...
    const filterGroupsClosed = this.useStore(
      (state) => state.filterGroupsClosed
    );
    ...
    const toggleFilterGroupOpen = this.useStore(
    ...
    const linkClass =
      'no-underline border-b-2 border-b-[rgba(2,102,112,0)] hover:border-b-[rgba(2,102,112,1)] transition-all';
    ...
    return (
      <div className="flex">
        ...
        <div className="w-full">
          ...
          <div
            className={
              this.mode === 'media' && count > 0
                ? 'lg:ml-5 grid xl:grid-cols-3 xl:gap-3 lg:grid-cols-2 lg:gap-2'
                : ''
            }
          >
            {count === 0 ? (
